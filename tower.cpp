...
static bool isRoad(const MapCode &value)
{
    return value == "a0";
}
...
int init(const char *body)
{
    ...
    ss.getline(buff, 256); // $$
    ss >> initHp >> initAttr.attack >> initAttr.defend >> buffInt >> buffInt >> initAttr.yellow >> initAttr.blue >> initAttr.red >> buffInt >> initX >> initY;
    // cout << initHp << ' ' << initAttr.attack << ' ' << initAttr.defend << ' ' << initAttr.yellow << ' ' << initAttr.blue << ' ' << initAttr.red << endl;
    ss.getline(buff, 256); // \n
    ss.getline(buff, 256); // $$
    for (int i = 0; i < N; i++)
    {
    ...
    while (strncmp(buff, "#", 1) != 0)
    {
        ...
        if (strncmp(buff, "mapcode=", 8) == 0)
        {
            ...
            attr.sp = atoi(buff + 3);
            // cout << mapCode << ' ' << attr.name << ' ' << attr.hp << ' ' << attr.attack << ' ' << attr.defend << ' ' << attr.sp << endl;
